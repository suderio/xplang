...
public class BigIntegerMath {
    ...
    static public int isqrt(final int n) {
        if (n < 0) throw new ArithmeticException("Negative argument " + n);
    ...
    static public BigInteger isqrt(final BigInteger n) {
        if (n.compareTo(BigInteger.ZERO) < 0) throw new ArithmeticException("Negative argument " + n);
        /* Start with an estimate from a floating point reduction.
         */
        BigInteger x;
        final int bl = n.bitLength();
        if (bl > 120) x = n.shiftRight(bl / 2 - 1);
        else {
    ...
    static public BigInteger iroot(final BigInteger x, final int n) {
        if (x.compareTo(BigInteger.ZERO) < 0) throw new ArithmeticException("Negative argument " + x);
        if (n < 1) throw new ArithmeticException("Non-positive argument " + n);
        /* Start with an estimate from a floating point reduction.
         */
        BigInteger r;
        final BigInteger nBig = BigInteger.valueOf(n);
        final int bl = x.bitLength();
        if (bl > 120) r = x.shiftRight(bl / n - 1);
        else {
    ...
    static public BigInteger[][] minor(final BigInteger[][] A, final int r, final int c) throws ArithmeticException {
        ...
        if (rL == 0) throw new ArithmeticException("zero row count in matrix");
        if (r < 0 || r >= rL) throw new ArithmeticException("row number " + r + " out of range 0.." + (rL - 1));
        /* original column count */
        final int cL = A[0].length;
        if (cL == 0) throw new ArithmeticException("zero column count in matrix");
        if (c < 0 || c >= cL) throw new ArithmeticException("column number " + c + " out of range 0.." + (cL - 1));
    ...
    static private BigInteger[][] colSubs(final BigInteger[][] A, final int c, final BigInteger[] v) throws ArithmeticException {
        ...
        for (int row = 0; row < rL; row++) {
            for (int col = 0; col < cL; col++) {
                 ...
                 */
                if (col != c) M[row][col] = A[row][col];
                else M[row][col] = v[row];
            }
        }
        return M;
    }
    ...
    static public BigInteger det(final BigInteger[][] A) throws ArithmeticException {
        ...
        /* column size */
        final int cL = A[0].length;
        if (cL != rL) throw new ArithmeticException("Non-square matrix dim " + rL + " by " + cL);

        /* Compute the low-order cases directly.
         */
        if (rL == 1) return A[0][0];
    ...
    static public Rational[] solve(final BigInteger[][] A, final BigInteger[] rhs) throws ArithmeticException {
        ...
        for (int c = 0; c < cL - 1; c++) {
            ...
            if (A[c][c].compareTo(BigInteger.ZERO) == 0) {
                 ...
                 */
                if (!swpd) throw new ArithmeticException("Zero determinant of main matrix");
            }
    ...
    static public BigInteger valueOf(final Vector<BigInteger> c, final BigInteger x) {
        if (c.isEmpty()) return BigInteger.ZERO;
    ...
    static public Rational centrlFactNumt(int n, int k) {
        if (k > n || k < 0 || (k % 2) != (n % 2)) return Rational.ZERO;
        else if (k == n) return Rational.ONE;
        else {
            /* Proposition 6.2.6 */
            ...
            for (int j = 0; j <= kprime; j++) {
                ...
                for (int nu = 0; nu <= j; nu++) {
                    ...
                    if (nu % 2 != 0) nusum = nusum.subtract(t);
                    else nusum = nusum.add(t);
                }
                nusum = nusum.divide(f.at(j)).divide(n + j);
                nusum = nusum.multiply(binomial(2 * kprime, kprime - j));
                if (j % 2 != 0) jsum = jsum.subtract(nusum);
                else jsum = jsum.add(nusum);
            }
    ...
    public BigInteger eulerPhi(final BigInteger n) {
        if (n.compareTo(BigInteger.ZERO) <= 0) throw new ArithmeticException("negative argument " + n + " of EulerPhi");
        final Ifactor prFact = new Ifactor(n);
        BigInteger phi = n;
        if (n.compareTo(BigInteger.ONE) > 0) for (int i = 0; i < prFact.primeexp.size(); i += 2) {
