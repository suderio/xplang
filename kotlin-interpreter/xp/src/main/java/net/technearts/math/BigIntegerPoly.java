...
public class BigIntegerPoly implements Cloneable {
    ...
    public BigIntegerPoly(final String L) throws NumberFormatException {
        ...
        sc.useDelimiter(",");
        while (sc.hasNextBigInteger()) a.add(sc.nextBigInteger());
        simplify();
    } /* ctor */
    ...
    public BigIntegerPoly(final BigInteger[] c) {
        for (BigInteger bigInteger : c) a.add(bigInteger.add(BigInteger.ZERO));
        simplify();
    } /* ctor */
    ...
    public BigInteger valueOf(final BigInteger x) {
        if (a.isEmpty()) return BigInteger.ZERO;
    ...
    public String toPString() {
        ...
        for (int n = 0; n < a.size(); n++) {
            ...
            if (num.compareTo(BigInteger.ZERO) != 0) {
                str.append(" ");
                if (num.compareTo(BigInteger.ZERO) > 0 && n > 0) str.append("+");
    ...
    public Vector<BigInteger> iroots() {
        ...
        /* collect the zero
         */
        if (a.firstElement().compareTo(BigInteger.ZERO) == 0) res.add(BigInteger.ZERO);
        ...
        if (a.elementAt(l).compareTo(BigInteger.ZERO) != 0) {
            ...
            for (int i = 0; i < cand.size(); i++) {
                ...
                if (roo.compareTo(BigInteger.ZERO) == 0)
                    /* found a root cand[i] */ res.add(cand.elementAt(i));
                roo = valueOf(cand.elementAt(i).negate());
                if (roo.compareTo(BigInteger.ZERO) == 0) res.add(cand.elementAt(i).negate());
            }
        }
        return res;
    } /* iroots */
    ...
    public Vector<BigIntegerPoly> ifactor() {
         ...
         */
        if (res[0].degree() > 0 || res[0].a.firstElement().compareTo(BigInteger.ONE) != 0) fac.add(res[0]);
        return fac;
    } /* ifactor */
