...
class Permutation implements Cloneable {
    ...
    Permutation(final int[] parts, int n) {
        images = new int[n];
        int el = 0;
        for (final int plen : parts) {
            for (int l = 0; l < plen; l++) {
                if (l == plen - 1) images[el] = el + 1 + REPOFFSET - plen;
                else images[el] = el + 1 + REPOFFSET;
                el++;
            }
        }
    ...
    static public void main(String[] args) {
        ...
        if (args.length == 1) {
            ...
            System.out.println("starting with " + p);
    ...
    private void init(int[] v) {
        ...
        System.arraycopy(v, 0, images, 0, v.length);
    ...
    Permutation multiply(final Permutation right) {
        if (images.length != right.images.length)
            throw new IllegalArgumentException("Different permutation lengths " + images.length + " vs " + right.images.length);
    ...
    public boolean isUnit() {
        for (int i = 0; i < images.length; i++)
            if (images[i] != i + REPOFFSET) return false;
        return true;
    } /* isUnit */
    ...
    public int[][] cycles() {
        ...
        while (true) {
             ...
             */
            if (notmoved == images.length) break;
             ...
             */
            if (tracei.length > 1) c.add(tracei);
        }
    ...
    public String cyclRep() {
        ...
        for (int[] ints : allcyc) {
            for (int e = 0; e < ints.length; e++) {
                if (e == 0) rep.append("(").append(ints[e] + 1 - REPOFFSET);
                else if (e == ints.length - 1) rep.append(" ").append(ints[e] + 1 - REPOFFSET).append(")");
                else rep.append(" ").append(ints[e] + 1 - REPOFFSET);
            }
        }
    ...
    int imageOf(final int e) {
         ...
         */
        if (e >= images.length + REPOFFSET) return e;
    ...
    public int compareTo(final Permutation oth) {
        if (images.length < oth.images.length) return -1;
        else if (images.length > oth.images.length) return +1;
        else {
            for (int j = 0; j < images.length; j++) {
                if (images[j] < oth.images[j]) return -1;
                else if (images[j] > oth.images[j]) return 1;
            }
            return 0;
        }
    } /* compareTo */
    ...
    public int smallestMoved() {
        for (int i = 0; i < images.length; i++)
            if (images[i] != i + REPOFFSET) return i;
        return images.length;
    } /* smallestMoved */
     ...
     */
    boolean isSorted() {
        if (orderByCycles() == 1) return true;
        ...
        if (cyc.length > 1) {
            ...
            for (int i = 0; i < cyc.length - 1; i++)
                for (int j = i + 1; j < cyc.length; j++) {
                    if (cyc[j].length < cyc[i].length) return false;
                }
                ...
                for (int j = i + 1; j < cyc.length; j++) {
                    for (int ei = 0; ei < cyc[i].length; ei++)
                        for (int ej = 0; ej < cyc[j].length; ej++)
                            if (cyc[j][ej] <= cyc[i][ei]) return false;
                }
            ...
            for (int i = 0; i < cyc.length - 1; i++) {
                for (int ei = 0; ei < cyc[i].length - 1; ei++)
                    if (cyc[i][ei + 1] <= cyc[i][ei]) return false;
            }
        }
        return true;
    } /* iscycSorted */
    ...
    public String toString() {
        ...
        for (int image : images) s.append((image + 1 - REPOFFSET)).append(" ");
        s.append(")");
        return s.toString();
    } /* toString */
