...
public class BigDecimalMath {
     ...
     * <a href="http://www.cs.arizona.edu/icon/oddsends/e.htm">...</a>
    ...
    /**
     * Euler's constant Pi.
     * <a href="http://www.cs.arizona.edu/icon/oddsends/pi.htm">...</a>
     */
     ...
     * <a href="http://www.worldwideschool.org/library/books/sci/math/MiscellaneousMathematicalConstants/chap35.html">...</a>
     */
     ...
     * <a href="http://www.worldwideschool.org/library/books/sci/math/MiscellaneousMathematicalConstants/chap58.html">...</a>
     */
     ...
     */
    static private final int TAYLOR_NTERM = 8;
    ...
    static public BigDecimal powRound(final BigDecimal x, final BigInteger n) {
        if (n.compareTo(Rational.MAX_INT) > 0 || n.compareTo(Rational.MIN_INT) < 0)
            throw new ProviderException("Not implemented: big power " + n);
        else
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (res.compareTo(p) > 0) {
                ...
                if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
                     ...
                     */
                    return cos(subtractRound(p.divide(new BigDecimal("2"), mc), res));
                } else {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0)
            ...
            if (res.compareTo(p) > 0) {
                ...
                if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
                     ...
                     */
                    return sin(subtractRound(p.divide(new BigDecimal("2"), mc), res));
                } else {
    ...
    static public BigDecimal asin(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ONE) > 0 || x.compareTo(BigDecimal.ONE.negate()) < 0) {
            ...
            return pi(mc).divide(new BigDecimal(2), mc);
        } else if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            BigDecimal pihalf = pi(mc).divide(new BigDecimal(2), mc);
    ...
    static public BigDecimal acosh(final BigDecimal x) {
        if (x.compareTo(BigDecimal.ONE) < 0)
            throw new ArithmeticException("Out of range argument cosh " + x);
        else if (x.compareTo(BigDecimal.ONE) == 0)
            return BigDecimal.ZERO;
        else {
            BigDecimal xhighpr = scalePrec(x, 2);
    ...
    static public BigDecimal Gamma(final Rational q, final MathContext mc) {
        if (q.isBigInteger()) {
            if (q.compareTo(Rational.ZERO) <= 0)
                throw new ArithmeticException("Gamma at " + q);
            else {
                /* Gamma(n) = (n-1)! */
    ...
    static public BigDecimal modpi(BigDecimal x) {
        ...
        BigDecimal onepi = pi(mc);
        BigDecimal pihalf = onepi.divide(new BigDecimal(2), mc);
