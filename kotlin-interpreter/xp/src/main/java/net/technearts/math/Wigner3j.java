...
public class Wigner3j {
    ...
    static public void main(String[] args) {
        if (args[0].compareTo("6j") == 0) {
            try {
                ...
                BigSurdVec w = wigner3j(m1, t1, t2, j.toString());
            ...
            try {
                ...
                System.out.println("CG factor sqrt" + (j3 + 1) + "sign " + ((-m3) / 2) + " " + w.toString());
            } catch (Exception e) {
                System.out.println(e.getMessage());
            }
        } else {
    ...
    static public BigSurdVec wigner3j(String m1, String t1, String t2, String j) {
         ...
         */
        java.util.Scanner s = new java.util.Scanner(m1);
        int m = s.nextInt();
        if (m % 3 != 0) throw new IllegalArgumentException("Angular momenta " + m + " not a multiple of three.");
         ...
         */
        s = new java.util.Scanner(j);
        int ji = 0;
        while (s.hasNextInt() && ji < m) {
            jvec[ji++] = s.nextInt();
            if (jvec[ji - 1] < 1) throw new IllegalArgumentException("Illegal value " + jvec[ji - 1] + " for 2j+1.");
        }

        /* the first two rows contain signed values of indices into the j list
         */
        s = new java.util.Scanner(t1);
        int ti = 0;
        while (s.hasNextInt()) tvec[ti++] = s.nextInt();
         ...
         */
        if (ji % 3 != 0) throw new IllegalArgumentException("j-count " + ji + " not a multiple of three.");
        if (ti != 2 * ji) throw new IllegalArgumentException("triad-count " + ti + " not twice j-count " + ji);
    ...
    static private BigSurdVec wigner3j(final int[] tvec, final Rational[] J, final Rational[] M, final int[] triadidx) {
        ...
        for (int t = 0; t < triadidx.length; t += 3) {
            ...
            if (J[triadidx[t]].subtract(J[triadidx[t + 1]]).abs().compareTo(J[triadidx[t + 2]]) > 0) return res;
        ...
        for (int i = 0; i < triadidx.length; i++) {
            ...
            if (M[triadidx[i]] == null) {
                ...
                if (M[triadidx[nei1]] != null && M[triadidx[nei2]] != null) {
                    ...
                    if (M[triadidx[nei1]] == null) wt = wt.multiply(J[triadidx[nei1]].multiply(2).add(1));
                    if (M[triadidx[nei2]] == null) wt = wt.multiply(J[triadidx[nei2]].multiply(2).add(1));
        ...
        if (freeM >= 0) {
            ...
            if (M[triadidx[freeM]] == null) {
                ...
                if (M[triadidx[nei1]] == null || M[triadidx[nei2]] == null) {
                     ...
                     */
                    if (tvec[nei1] < 0) m1 = m1.negate();
                    if (tvec[nei2] < 0) m2 = m2.negate();
                    /* m3 = -(m1+m2) */
        ...
        for (int ji = 0; ji < triadidx.length; ji += 3) {
             ...
             */
            if (tvec[ji] < 0) m1 = m1.negate();
            if (tvec[ji + 1] < 0) m2 = m2.negate();
            if (tvec[ji + 2] < 0) m3 = m3.negate();
             ...
             */
            if (res.signum() == 0) return BigSurdVec.ZERO;
        }
        ...
        if (sig.a.abs().testBit(0)) res = res.negate();
        return res;
    } /* wigner3j */
    ...
    static protected BigSurd wigner3jm(Rational j1, Rational j2, Rational j3, Rational m1, Rational m2, Rational m3) {
         ...
         */
        if (m1.add(m2).add(m3).signum() != 0) return BigSurd.ZERO;

        /* Check that j1+j2+j3 is integer
         */
        if (!j1.add(j2).add(j3).isBigInteger()) return BigSurd.ZERO;

        /* Check that |j1-j2|<=j3 <= |j1+j2|
         */
        Rational j1m2 = j1.subtract(j2);
        if (j1m2.abs().compareTo(j3) > 0) return BigSurd.ZERO;
        Rational j1p2 = j1.add(j2);
        if (j1p2.abs().compareTo(j3) < 0) return BigSurd.ZERO;

        /* Check that |m_i| <= j_i
         */
        if (m1.abs().compareTo(j1) > 0 || m2.abs().compareTo(j2) > 0 || m3.abs().compareTo(j3) > 0) return BigSurd.ZERO;
        ...
        while (true) {
            ...
            if (k % 2 == 0) sumk = sumk.add(new Rational(BigInteger.ONE, d));
            else sumk = sumk.subtract(new Rational(BigInteger.ONE, d));
            j1j2jk--;
            j1m1k--;
            j2m2k--;
            jj2m1k++;
         ...
         */
        if (j1m2.subtract(m3).intValue() % 2 != 0) sumk = sumk.negate();
