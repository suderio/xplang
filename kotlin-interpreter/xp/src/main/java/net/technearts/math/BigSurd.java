...
public class BigSurd implements Cloneable, Comparable<BigSurd> {
    ...
    public BigSurd(Rational a, Rational b) {
         ...
         */
        if (b.signum() < 0) throw new ProviderException("Not implemented: imaginary surds");
        this.disc = b;
        normalize();
        normalizeG();
    } /* ctor */
    ...
    public BigSurdVec add(final BigSurd val) {
         ...
         */
        if (signum() == 0) return new BigSurdVec(val);
        else if (val.signum() == 0) return new BigSurdVec(this);
        else
    ...
    public BigSurd divide(final BigSurd val) {
        if (val.signum() == 0) throw new ArithmeticException("Dividing " + this + " through zero.");
    ...
    public int compareTo(final BigSurd val) {
        ...
        if (sig < 0 && sigv >= 0) return -1;
        if (sig > 0 && sigv <= 0) return 1;
        if (sig == 0 && sigv == 0) return 0;
        if (sig == 0 && sigv > 0) return -1;
        if (sig == 0 && sigv < 0) return 1;
        ...
        if (c == 0) return 0;
             ...
             */
        else if (sig > 0 && c > 0 || sig < 0 && c < 0) return 1;
        else return -1;
    } /* BigSurd.compareTo */
    ...
    public Rational toRational() {
        if (isRational()) return pref;
        else throw new ArithmeticException("Undefined conversion " + this + " to Rational.");
    } /* BigSurd.toRational */
