...
public class Ifactor implements Cloneable, Comparable<Ifactor> {
    ...
    protected Ifactor multGcdLcm(final Ifactor oth, int type) {
         ...
         */
        if (!primeexp.isEmpty() && !oth.primeexp.isEmpty()) {
            ...
            else if (primeexp.firstElement() == 1 && type == 2) return oth;
            else if (primeexp.firstElement() == 1 && type == 1) return this;
            else if (oth.primeexp.firstElement() == 1 && type == 0) return this;
            else if (oth.primeexp.firstElement() == 1 && type == 2) return this;
            else if (oth.primeexp.firstElement() == 1 && type == 1) return oth;
            else {
                int idxThis = 0;
                int idxOth = 0;
                switch (type) {
                                ...
                                ex = switch (type) {
                                    case 0 ->
                                        /* product means adding exponents */
                                            primeexp.elementAt(idxThis + 1) + oth.primeexp.elementAt(idxOth + 1);
                                    case 1 ->
                                        /* gcd means minimum of exponents */
                                            Math.min(primeexp.elementAt(idxThis + 1), oth.primeexp.elementAt(idxOth + 1));
                                    default ->
                                        /* lcm means maximum of exponents */
                                            Math.max(primeexp.elementAt(idxThis + 1), oth.primeexp.elementAt(idxOth + 1));
                                };
                                prod.primeexp.add(p);
    ...
    public Ifactor add(final BigInteger oth) {
         ...
         */
        if (oth.compareTo(BigInteger.ZERO) != 0) return new Ifactor(n.add(oth));
        else return this;
    } /* Ifactor.add */
    ...
    public Ifactor pow(final int exponent) throws ArithmeticException {
         ...
         */
        if (exponent < 0) throw new ArithmeticException("Cannot raise " + this + " to negative " + exponent);
        else if (exponent == 0) return new Ifactor(1);
    ...
    public Rational root(final int r) throws ArithmeticException {
        if (r == 0) throw new ArithmeticException("Cannot pull zeroth root of " + this);
        else if (r < 0) {
            /* a^(-1/b)= 1/(a^(1/b))
             */
            ...
            for (int i = 0; i < primeexp.size(); i += 2) {
                 ...
                 */
                int ex = primeexp.elementAt(i + 1);
                if (ex % r != 0) throw new ArithmeticException("Cannot pull " + r + "th root of " + this);
    ...
    public Ifactor sigma(int k) {
         ...
         */
        if (n.compareTo(BigInteger.ONE) == 0) return ONE;
        else if (n.compareTo(BigInteger.ZERO) == 0) return ZERO;
        else {
            ...
            for (int i = 0; i < primeexp.size(); i += 2) {
                int ex = primeexp.elementAt(i + 1);
                if (k == 0) resul = resul.multiply(ex + 1);
                else {
                    Integer p = primeexp.elementAt(i);
    ...
    public boolean issquare() {
        ...
        for (int i = 1; i < primeexp.size(); i += 2) {
            if (primeexp.elementAt(i) % 2 != 0) return false;
        }
        return true;
    } /* Ifactor.issquare */
    ...
    public Ifactor max(final Ifactor oth) {
        if (n.compareTo(oth.n) >= 0) return this;
        else return oth;
    } /* Ifactor.max */
    ...
    public String toString() {
        StringBuilder resul = new StringBuilder(n.toString() + ":");
        if (n.compareTo(BigInteger.ONE) == 0) resul.append("1");
        else {
            boolean firstMul = true;
            for (int i = 0; i < primeexp.size(); i += 2) {
                if (!firstMul) resul.append("*");
                if (primeexp.elementAt(i + 1) > 1)
                    resul.append(primeexp.elementAt(i).toString()).append("^").append(primeexp.elementAt(i + 1).toString());
                else resul.append(primeexp.elementAt(i).toString());
                firstMul = false;
            }
        }
