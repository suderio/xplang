...
class RatPoly {
    ...
    public RatPoly(final String L) throws NumberFormatException {
        a = new Vector<>();
        java.util.Scanner sc = new Scanner(L);
        sc.useDelimiter(",");
        while (sc.hasNext()) {
            String tok = sc.next();
    ...
    public RatPoly(final Vector<BigInteger> A, final Vector<BigInteger> B) {
        ...
        for (int j = 0; j < A.size(); j++) {
            if (A.elementAt(j).compareTo(BigInteger.ZERO) <= 0) {
                if (Nmax.compareTo(BigInteger.ZERO) < 0) Nmax = A.elementAt(j).negate();
                else Nmax = Nmax.min(A.elementAt(j).negate());
            }
        }
        if (Nmax.compareTo(BigInteger.ZERO) < 0)
            throw new ArithmeticException("Infinite Number of Terms in Series " + Nmax);
    ...
    public Rational at(final int n) {
        if (n < a.size()) return (a.elementAt(n));
    ...
    public void set(final int n, final Rational value) {
        if (n < a.size()) a.set(n, value);
        else {
            /* fill intermediate powers with coefficients of zero
             */
            while (a.size() < n) a.add(new Rational(0, 1));
            a.add(value);
        }
    } /* set */
    ...
    public int ldegree() {
        for (int n = 0; n < a.size(); n++)
            if (a.elementAt(n).compareTo(BigInteger.ZERO) != 0) return n;
        return 0;
    } /* ldegree */
    ...
    public RatPoly multiply(final Rational val) {
        ...
        if (val.compareTo(BigInteger.ZERO) != 0) for (int n = 0; n < a.size(); n++)
    ...
    public RatPoly pow(final int n) throws ArithmeticException {
        ...
        if (n < 0) throw new ArithmeticException("negative polynomial power " + n);
        else {
    ...
    public RatPoly add(final RatPoly val) {
         ...
         */
        final int nmax = Math.max(degree(), val.degree());
        for (int n = 0; n <= nmax; n++) {
    ...
    public RatPoly divide(final Rational val) {
        if (val.compareTo(Rational.ZERO) != 0) {
            ...
            return resul;
        } else throw new ArithmeticException("Cannot divide " + toPString() + " through zero.");
    } /* divide */
    ...
    public RatPoly divide(final RatPoly val, int nmax) {
         ...
         */
        while (num.at(0).compareTo(BigInteger.ZERO) == 0 && val.at(0).compareTo(BigInteger.ZERO) == 0) {
            num.a.remove(0);
            val.a.remove(0);
            if (num.size() <= 1 || val.size() <= 1) break;
        }
        ...
        for (int n = 0; n <= nmax; n++) {
            ...
            for (int nres = 0; nres < n; nres++) {
                coef = coef.subtract(resul.at(nres).multiply(val.at(n - nres)));
            }
            coef = coef.divide(val.at(0));
            resul.set(n, coef);
        }
        resul.simplify();
        return (resul);
    ...
    public String toString() {
        ...
        for (int n = 0; n < a.size(); n++) {
            if (n == 0) str.append(a.elementAt(n).toString());
            else str.append(",").append(a.elementAt(n).toString());
        }
        /* print at least a sole zero
         */
        if (str.isEmpty()) str = new StringBuilder("0");
        return str.toString();
    } /* toString */
    ...
    public String toPString() {
        ...
        for (int n = 0; n < a.size(); n++) {
            ...
            if (num.compareTo(BigInteger.ZERO) != 0) {
                str.append(" ");
                if (num.compareTo(BigInteger.ZERO) > 0) str.append("+");
                str.append(a.elementAt(n).toString());
                if (n > 0) {
                    str.append("*x");
                    if (n > 1) str.append("^").append(n);
                }
            }
        }
        /* print at least a sole zero
    ...
    private void simplify() {
        int n = a.size() - 1;
        if (n >= 0) while (a.elementAt(n).compareTo(BigInteger.ZERO) == 0) {
            a.remove(n);
            if (--n < 0) break;
        }
    } /* simplify */
    ...
    public RatPoly mobiusTInv(int maxdeg) {
        ...
        for (int i = 1; i <= maxdeg; i++) {
            Rational c = new Rational();
            for (int d = 1; d <= i && d < a.size(); d++) {
                if (i % d == 0) c = c.add(a.elementAt(d));
            }
            r.set(i, c);
        }
        r.simplify();
    ...
    public RatPoly binomialTInv(int maxdeg) {
        ...
        for (int i = 0; i <= maxdeg; i++) {
            Rational c = new Rational(0, 1);
            for (int j = 0; j <= i && j < a.size(); j++)
                if ((j + i) % 2 != 0) c = c.subtract(a.elementAt(j).multiply(BigIntegerMath.binomial(i, j)));
                else c = c.add(a.elementAt(j).multiply(BigIntegerMath.binomial(i, j)));
            r.set(i, c);
        }
        r.simplify();
        return r;
    ...
    public Vector<BigComplex> roots(int digits) {
        ...
        while (!convr) {
            ...
            for (int v = 0; v < d; v++) {
                ...
                for (int j = 0; j < d; j++) {
                    if (j != v) nv = nv.divide(thisx.subtract(res.elementAt(j)), mc);
                }

                /* is this value converged ? */
                if (nv.abs(mc).doubleValue() > thisx.abs(mc).doubleValue() * Math.pow(10.0, -digits)) convr = false;

                thisx = thisx.subtract(nv);

                /* If unstable, start over */
                if (thisx.abs(MathContext.DECIMAL32).doubleValue() > randRad) return roots(digits);
