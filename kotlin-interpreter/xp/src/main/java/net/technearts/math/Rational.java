...
public class Rational implements Cloneable, Comparable<Rational> {
    ...
    public Rational(Vector<BigInteger> cfr) {
        if (cfr.isEmpty()) throw new NumberFormatException("Empty continued fraction");
        else if (cfr.size() == 1) {
    ...
    public static Rational binomial(Rational n, BigInteger m) {
        if (m.compareTo(BigInteger.ZERO) == 0) return Rational.ONE;
        Rational bin = n;
    ...
    public static Rational hankelSymb(Rational n, int k) {
        if (k == 0) return Rational.ONE;
        else if (k < 0) throw new ArithmeticException("Negative parameter " + k);
    ...
    static public BigInteger lcmDenom(final Rational[] vals) {
        ...
        for (Rational val : vals) l = BigIntegerMath.lcm(l, val.b);
        return l;
    } /* Rational.lcmDenom */
    ...
    public Rational pow(int exponent) {
        ...
        if (exponent > 0) return (new Rational(num, deno));
        else return (new Rational(deno, num));
    } /* Rational.pow */
    ...
    public Rational pow(BigInteger exponent) throws NumberFormatException {
        ...
        if (exponent.compareTo(MAX_INT) > 0)
            throw new NumberFormatException("Exponent " + exponent + " too large.");
        if (exponent.compareTo(MIN_INT) < 0)
            throw new NumberFormatException("Exponent " + exponent + " too small.");
    ...
    public Rational root(BigInteger r) throws NumberFormatException {
        /* test for overflow */
        if (r.compareTo(MAX_INT) > 0) throw new NumberFormatException("Root " + r + " too large.");
        if (r.compareTo(MIN_INT) < 0) throw new NumberFormatException("Root " + r + " too small.");

        int rthroot = r.intValue();
        /* cannot pull root of a negative value with even-valued root */
        if (compareTo(ZERO) < 0 && (rthroot % 2) == 0)
            throw new NumberFormatException("Negative basis " + this + " with odd root " + r);
        ...
        if (flipsign) return resul.negate();
        else return resul;
    } /* Rational.root */
    ...
    public Rational pow(Rational exponent) throws NumberFormatException {
        if (exponent.a.compareTo(BigInteger.ZERO) == 0) return new Rational(1, 1);
    ...
    public Rational divide(final Rational val) {
        if (val.compareTo(Rational.ZERO) == 0)
            throw new ArithmeticException("Dividing " + this + " through zero.");
    ...
    public Rational divide(int val) {
        if (val == 0) throw new ArithmeticException("Dividing " + this + " through zero.");
    ...
    public BigInteger floor() {
        ...
        else if (a.compareTo(BigInteger.ZERO) > 0) return a.divide(b);
        else return a.divide(b).subtract(BigInteger.ONE);
    } /* Rational.floor */
    ...
    public BigInteger ceil() {
         ...
         */
        if (b.compareTo(BigInteger.ONE) == 0) return a;
        else if (a.compareTo(BigInteger.ZERO) > 0) return a.divide(b).add(BigInteger.ONE);
    ...
    public String toString() {
        if (b.compareTo(BigInteger.ONE) != 0) return (a.toString() + "/" + b.toString());
    ...
    public Rational Pochhammer(final BigInteger n) {
        ...
        else if (n.compareTo(BigInteger.ZERO) == 0) return Rational.ONE;
        else {
     ...
     */
    int intValue() {
        if (!isInteger()) throw new NumberFormatException("cannot convert " + this + " to integer.");
        return a.intValue();
    }
    ...
    BigInteger BigIntegerValue() {
        if (!isBigInteger()) throw new NumberFormatException("cannot convert " + this + " to BigInteger.");
        return a;
    }
    ...
    public Vector<BigInteger> cfrac() {
        if (signum() < 0) throw new NumberFormatException("Unsupported cfrac for negative " + this);
        Vector<BigInteger> cf = new Vector<>();
        if (signum() != 0) {
            BigInteger[] nRem = a.divideAndRemainder(b);
            cf.add(nRem[0]);
            /* recursive call : this = nRem[0]+nRem[1]/b = nRem[0] + 1/(b/nRem[1])
             */
            if (nRem[1].signum() != 0) cf.addAll((new Rational(b, nRem[1])).cfrac());
        }
        return cf;
    } /* Rational.cfrac */
